 ...
 */
    ...
    void PopulateAllTypes(Aquarium *aquarium)
    {
        ...
        auto fish2 = make_shared<Molly>(aquarium);
        fish2->SetLocation(400, 400);
        aquarium->Add(fish2);
        ...
        auto fish4 = make_shared<StarFish>(aquarium);
    ...
    void TestAllTypes(wxString filename)
    {
        ...
        // Ensure the types are correct
        ASSERT_TRUE(regex_search(xml,
                                 wregex(L"<aqua><item.* type=\"beta\"/><item.* type=\"molly\"/><item.* type=\"castle\"/><item.* type=\"starfish\"/></aqua>")));
    };
};
